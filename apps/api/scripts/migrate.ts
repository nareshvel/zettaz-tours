...
export async function migrate(adminUrl: string, runtimeRole: string) {
  ...
  try {
    ...
    await tx.query(
      `GRANT SELECT,INSERT,UPDATE,DELETE ON tenants,staff_users,memberships,products,schedules,departures,holds,bookings,outbox_events,pickup_locations,departure_pickup_plans,pickup_stops,waiver_templates TO ${role}`,
    );
    await tx.query(
      `GRANT SELECT,INSERT ON booking_changes,booking_change_quotes,payments,price_snapshots,audit_events,event_receipts,idempotency_keys,waiver_signatures TO ${role}`,
    );
