...
function commissionSummary(p: Partner, fallback: string) {
  ...
  const dir =
    p.commission_direction === "partner_owes_tenant"
      ? "partner pays us"
      : "we pay partner";
...
function PartnerFormModal({
  ...
  const [form, setForm] = useState<PartnerForm>(() =>
    formFromPartner(partner, fallback),
  );
  ...
  function commissionPayload() {
    ...
    if (
      ...
      p.commission_amount_minor = Math.round(
  ...
  return (
    ...
    >
      ...
      <div className={showRate || showAmount ? "form-grid three" : "form-grid"}>
        ...
        {showAmount && (
          <Field
            label={`Amount (${form.commissionCurrency || fallback})`}
            required
          >
            <input
              required
...
function SettlementStepper({
  ...
  const currentIdx = PIPELINE_STEPS.indexOf(
    settlement.status as (typeof PIPELINE_STEPS)[number],
  );
    ...
    <div className="settlement-pipeline">
      {PIPELINE_STEPS.map((step, i) => {
        ...
        return (
          <div
            key={step}
            className={`pipeline-step ${done ? "done" : active ? "active" : isVoided ? "voided" : "future"}`}
          >
...
function GenerateDialog({
  ...
  const { data: preview } = useResource<{
    count: number;
    total_minor: number;
    currency: string;
  }>(
    open
      ? `finance/v1/partners/${partner.id}/bookings/unsettled-summary?from=${periodStart}&to=${periodEnd}`
      : null,
  );
        ...
        (noBookings ? (
          <div
            style={{
              ...
              border:
                "1px solid color-mix(in srgb, var(--warning, #f59e0b) 35%, transparent)",
              borderRadius: 8,
              padding: "10px 14px",
              fontSize: "0.82rem",
          ...
          >
            ...
            No unsettled bookings found in this period. Adjust the dates to
            include bookings.
          </div>
        ) : (
          <div
            style={{
              ...
              gap: 8,
              background:
                "color-mix(in srgb, var(--success, #10b981) 10%, transparent)",
              border:
                "1px solid color-mix(in srgb, var(--success, #10b981) 30%, transparent)",
              borderRadius: 8,
              padding: "10px 14px",
              fontSize: "0.82rem",
            }}
          >
            ...
            <span>
              <strong>{preview.count}</strong> unsettled booking
              {preview.count !== 1 ? "s" : ""} ·{" "}
              <strong>
                {money(preview.total_minor, preview.currency || currency)}
              </strong>
            </span>
          </div>
        ))}
...
function MarkPaidDialog({
  ...
  async function submit() {
    ...
    if (confirmedAmount)
      body.confirmed_amount_minor = Math.round(Number(confirmedAmount) * 100);
  ...
  return (
    ...
    >
      ...
      <Field
        ...
        hint="Bank transfer ref, cheque number, or remittance advice."
      >
        <input
      ...
      <Field
        ...
        hint={`Leave blank if exact match (${settlement ? money(settlement.net_minor, settlement.currency) : ""})`}
      >
        <input
          type="number"
          step="0.01"
...
function AdvanceDialog({
  ...
  const titleMap: Record<string, string> = {
    invoiced: "Issue Settlement",
    sent: "Mark as Sent",
    voided: "Void Settlement",
  };

  return (
    <FormDialog
      open={open && !!settlement}
      title={titleMap[effectiveStatus] ?? "Advance Settlement"}
      busy={advance.busy}
      error={advance.error}
      onSubmit={submit}
      onClose={onClose}
    ...
    >
      {effectiveStatus === "invoiced" && (
        <Field
          ...
          hint="Reference number you'll send to the partner."
        >
          <input
            value={invoiceNumber}
      ...
      {effectiveStatus === "sent" && (
        <p style={{ fontSize: "0.82rem", color: "var(--muted)", margin: 0 }}>
          Confirm you've sent the statement to{" "}
          <strong>{settlement?.currency}</strong> partner. You can record the
  ...
  return (
    <tr>
      <td>
        {dateOnly(
          entry.event_at,
          dateFormat as "DD/MM/YYYY" | "MM/DD/YYYY" | "YYYY-MM-DD",
        )}
      </td>
      <td>
        {entry.description}
      ...
      <td>
        <span className={`ledger-badge ${entry.status ?? ""}`}>
          {entry.status ? label(entry.status) : "—"}
        </span>
      </td>
      <td className="num dr">
        {entry.dr_minor > 0 ? money(entry.dr_minor, entry.currency) : "—"}
      </td>
      <td className="num cr">
        {entry.cr_minor > 0 ? money(entry.cr_minor, entry.currency) : "—"}
      </td>
...
function PartnerDetailPanel({
  ...
  const [highlightSettlement, setHighlightSettlement] =
    useState<Settlement | null>(null);
    ...
    statusFilter === "unsettled"
      ...
      : statusFilter === "settled"
        ...
        : // For settlement pipeline statuses, we show settled bookings + filter by settlement status client-side
          ["draft", "invoiced", "sent", "paid", "voided"].includes(statusFilter)
          ? "settled"
          : "";
  ...
  const {
    ...
    error: ledgerError,
    reload: reloadLedger,
  } = useResource<LedgerResponse>(
    `finance/v1/partners/${partner.id}/ledger?${ledgerQs}`,
  );
  const { data: settlementData, reload: reloadSettlements } = useResource<
    Settlement[]
  >(`finance/v1/partners/${partner.id}/settlements`);
  ...
  const fallback =
    ...
    session.tenant.config.collectionCurrency;
  const currency =
    ledger?.partner.commission_currency ||
      ...
      <div className="finance-detail-toolbar">
        <div className="finance-detail-toolbar-title">
          ...
          {partner.partner_type && (
            <span className="ledger-badge settled" style={{ marginLeft: 8 }}>
              {label(partner.partner_type)}
            </span>
          )}
          {directionLabel && (
            <span className="partner-dir-chip" title={directionLabel}>
              {partner.commission_direction === "partner_owes_tenant"
                ? "Dr"
                : "Cr"}
            </span>
          ...
          {partner.status === "inactive" && (
            <span className="ledger-badge voided" style={{ marginLeft: 6 }}>
              Inactive
            </span>
          )}
        </div>
      ...
      <div className="finance-detail-scroll">
        ...
        <div className="finance-account-strip">
          <div className="finance-account-tile">
            ...
            <div
              className={`finance-account-tile-value ${balance > 0 ? "receivable" : balance < 0 ? "payable" : ""}`}
            >
              {balance === 0 ? (
          ...
          <div className="finance-account-tile">
            ...
            <div
              className="finance-account-tile-value"
              style={{ color: unsettledCount > 0 ? "#d97706" : "var(--ink)" }}
            >
          ...
          <div className="finance-account-tile">
            ...
            >
              {latestSettlement ? (
                <span className={`ledger-badge ${latestSettlement.status}`}>
                  {label(latestSettlement.status)}
                </span>
              ) : (
          ...
          <div className="finance-account-tile">
            ...
            >
              {partner.commission_type ? (
                <>
                  Commission:{" "}
                  <strong>{commissionRate(partner, fallback)}</strong>
                </>
              ) : (
            ...
            >
              {partner.settlement_schedule
                ? label(partner.settlement_schedule)
                : "Manual"}
              {partner.payment_terms_days
                ? ` · Net ${partner.payment_terms_days}d`
                : ""}
            </div>
          </div>
        </div>
        ...
        {latestSettlement &&
          ...
          latestSettlement.status !== "voided" && (
            <div className="settlement-pipeline-card">
              ...
              >
                <span style={{ color: "var(--muted)", fontWeight: 500 }}>
                  {dateOnly(
                    latestSettlement.period_start,
                    session.tenant.config.dateFormat as
                      "DD/MM/YYYY" | "MM/DD/YYYY" | "YYYY-MM-DD",
                  )}
                  {" – "}
                  {dateOnly(
                    latestSettlement.period_end,
                    session.tenant.config.dateFormat as
                      "DD/MM/YYYY" | "MM/DD/YYYY" | "YYYY-MM-DD",
                  )}
                </span>
              ...
              <SettlementStepper
                ...
                onAdvance={(toStatus) =>
                  handlePipelineStep(latestSettlement, toStatus)
                }
              />
              <div
                style={{
              ...
              >
                <span>
                  Click the next step to advance → it opens a confirmation
                  dialog.
                </span>
        ...
        {ledger?.entries.length === 0 && (
          <Empty title="No transactions">
            {filterCount > 0
              ...
              : "Attributed bookings and settlements will appear here."}
          </Empty>
        )}
              ...
              <table className="ledger-table">
                <thead>
                  <tr>
                    ...
                    <th>Description</th>
                    <th>Status</th>
                    <th className="num" title="Dr — partner owes you">
                      Dr
                    </th>
                    <th className="num" title="Cr — you owe partner">
                      Cr
                    </th>
      ...
      {deletingSettlement && (
        ...
        >
          <p style={{ margin: 0 }}>
            ...
            <strong>
              {dateOnly(
                deletingSettlement.period_start,
                session.tenant.config.dateFormat as any,
              )}{" "}
              –{" "}
              {dateOnly(
                deletingSettlement.period_end,
                session.tenant.config.dateFormat as any,
              )}
            </strong>{" "}
            and restore all {deletingSettlement.booking_count ?? ""} bookings to
            unsettled. This cannot be undone.
          </p>
        </FormDialog>
      )}
    </>
...
export function FinancePartners({
  ...
  const { data: partners, error: partnersError } = useResource<Partner[]>(
    `finance/v1/partners?_r=${listRev}`,
  );
      ...
      <div className="finance-detail-panel">
        {!selectedPartner ? (
          <>
            <div className="finance-detail-toolbar">
              <div
                className="finance-detail-toolbar-title"
            ...
            <div className="finance-detail-empty">
              <Landmark size={48} />
              <p>
                Select a partner from the list to view their transaction
                register and settlement history.
              </p>
            </div>
          </>
        ) : (
