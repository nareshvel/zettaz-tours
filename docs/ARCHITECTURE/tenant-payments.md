...
For Stripe Connect, **direct charges are the accepted starting point for Rock Adventures** because the tenant operates its own business and customer relationship. Use an Accounts v2 merchant configuration with full Dashboard access, verify merchant capability status before collection, and retain connected-account context on provider objects and webhooks. Other tenants remain configurable and require their own eligibility and responsibility evidence. See [ADR 016](../DECISIONS/016-rock-launch-operations.md).

Before live enablement: validate platform and tenant countries, merchant onboarding and required capabilities, currencies/settlement bank, who pays processing fees, liability for losses/refunds/disputes, payout control, and any Zettaz transaction fee. Prefer provider-hosted/embedded onboarding and account-health remediation to collecting identity documents ourselves.
