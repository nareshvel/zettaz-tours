...
- **One Zettaz Crew binary.** Phone field, tablet dock, and Pay are densities of `apps/mobile`, not separate apps. Empty Today is correct when the actor is unassigned. Phased delivery: [../STRATEGY/crew-app-delivery.md](../STRATEGY/crew-app-delivery.md). GPS, card-present, consumer app, and a white-label second binary remain Track B. GPS expansion (held, Connecteam study 19 Sep): [../STRATEGY/crew-gps-later.md](../STRATEGY/crew-gps-later.md).
...
- **Separate payment domains.** Cursor Stripe plugin (20 Sep): **Zettaz SaaS** = live Stripe Billing; **Zettaz Pay** = live Connect platform. Owner: proceed live; do not mix keys or webhooks. **Zettaz Pay** is the gateway brand guests/tenants see. Charge pattern remains tenant merchant of record with direct charges (Accounts v2, full Dashboard). Platform application fee **1%** (`ZETTAZ_PAY_APPLICATION_FEE_BPS=100`), changeable without mixing products. Card-present, eligibility, disputes, and the XCD rate snapshot remain open. See [../DECISIONS/016-rock-launch-operations.md](../DECISIONS/016-rock-launch-operations.md).
...
- **Booking integrations** (renamed from Integrations): sub-tabs Channels / Product mapping / Inbound queue / Import; one row per channel; channel search and empty states for mapping/inbox/import (19 Sep).
