...
export class NotificationService {
  ...
  /** Insert a notification row inside an existing command transaction. */
  async queueInTransaction(
    ...
    const totalMinor = Number(booking.total_minor ?? 0);
    const paidMinor = Number(booking.paid_minor ?? 0);
    ...
    const result: QueuedNotification = {
      ...
      kind,
      channel: "email",
      recipient: booking.lead_email as string,
      locale,
  ...
  async request(actor: Actor, bookingId: string, key: string, raw: unknown) {
    ...
    const created = await this.db.command(
      ...
      input,
      async (tx) =>
        this.queueInTransaction(
          tx,
          actor,
          bookingId,
          input.kind as CustomerNotificationKind,
        ),
    );
    ...
    return this.db
      .command(
        ...
        async (tx) => {
          ...
          if (row.status === "sent")
            throw new BadRequestException(
              "This communication was already sent.",
            );
  ...
  async deliver(actor: Actor, bookingId: string, messageId: string) {
