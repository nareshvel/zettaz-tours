import { BadRequestException, Body, Controller, Get, Headers, Injectable, Param, Post, Query } from "@nestjs/common";
...
const tripEvent = z.object({ state: z.enum(["preparing","en_route_pickup","boarding","departed","at_stop","delayed","completed","cancelled","emergency"]), reason: z.string().trim().min(3).max(500).optional() }).strict();
...
export class CrewService {
  ...
  event(actor: Actor, departure: string, key: string, raw: unknown) {
    ...
    return this.db.command(actor, `trip.run:${departure}`, key, input, async (tx) => {
      const assigned = await tx.query("SELECT 1 FROM departure_assignments WHERE tenant_id=$1 AND crew_actor_id=$2 AND departure_id=$3 AND status='active'", [actor.tenantId, actor.actorId, departure]);
      if (!assigned.rowCount) throw new BadRequestException("Crew can update only an assigned departure");
      const prior = (await tx.query("SELECT state,version FROM trip_runs WHERE tenant_id=$1 AND departure_id=$2 FOR UPDATE", [actor.tenantId, departure])).rows[0] as { state: string; version: number } | undefined;
      if (["completed","cancelled"].includes(prior?.state ?? "")) throw new BadRequestException("Completed or cancelled trip runs cannot be changed");
      const version = (prior?.version ?? 0) + 1;
      await tx.query("INSERT INTO trip_runs(tenant_id,departure_id,state,version,recorded_by) VALUES($1,$2,$3,$4,$5) ON CONFLICT(tenant_id,departure_id) DO UPDATE SET state=EXCLUDED.state,version=EXCLUDED.version,recorded_by=EXCLUDED.recorded_by,updated_at=clock_timestamp()", [actor.tenantId, departure, input.state, version, actor.actorId]);
      const result = { id: randomUUID(), departureId: departure, state: input.state, version, reason: input.reason ?? null };
      await tx.query("INSERT INTO trip_run_events(tenant_id,id,departure_id,state,reason,actor_id) VALUES($1,$2,$3,$4,$5,$6)", [actor.tenantId, result.id, departure, input.state, result.reason, actor.actorId]);
      await record(tx, actor, "trip_run.event_recorded", departure, prior ?? null, result, input.reason);
      return result;
    });
  }
}
...
export class CrewController {
  ...
  @Post("departures/:id/events") @Access("crew.trip.read") event(@CurrentActor() actor: Actor, @Param("id") value: string, @Headers("idempotency-key") key: string, @Body() body: unknown) {
    return this.service.event(actor, parse(departureId, value), parse(keySchema, key), body);
  }
}
