  ...
  detail(actor: Actor, customerId: string) {
    return this.db.transaction(actor, async (tx) => {
      ...
      const payments = await tx.query(
        "SELECT booking_id AS booking_id,'payment' AS kind,occurred_at,amount_minor,currency,status,method FROM payments WHERE tenant_id=$1 AND booking_id=ANY($2::uuid[])",
        [actor.tenantId, bookingIds],
      );
      const waivers = await tx.query(
        "SELECT booking_id,'waiver' AS kind,occurred_at,signer_name,signer_capacity FROM waiver_signatures WHERE tenant_id=$1 AND booking_id=ANY($2::uuid[])",
        [actor.tenantId, bookingIds],
      );
      const messages = await tx.query(
        "SELECT booking_id,'message' AS kind,requested_at AS occurred_at,subject,status,channel FROM notification_messages WHERE tenant_id=$1 AND booking_id=ANY($2::uuid[])",
        [actor.tenantId, bookingIds],
      );
      const changes = await tx.query(
        "SELECT booking_id,kind,occurred_at,reason FROM booking_changes WHERE tenant_id=$1 AND booking_id=ANY($2::uuid[])",
        [actor.tenantId, bookingIds],
      );
...
export class CustomerController {
  ...
  @Get() @Access("bookings.read") list(
    @CurrentActor() actor: Actor,
    @Query() query: unknown,
  ) {
    return this.service.list(actor, query);
  }
  @Get(":id") @Access("bookings.read") detail(
    @CurrentActor() actor: Actor,
