...
const passengerSchema = z
  .object({
    ...
    identityPending: z.boolean().default(false),
  })
  .strict();
const rosterSchema = z
  .object({ passengers: z.array(passengerSchema).min(1).max(100) })
  .strict();
const correctionSchema = rosterSchema
...
export class PassengerService {
  ...
  list(actor: Actor, bookingId: string) {
    return this.db.transaction(actor, async (tx) => {
      ...
      return (
        await tx.query(
          `SELECT p.id,p.name,p.category,p.is_minor,p.identity_pending,p.guardian_passenger_id,p.roster_version,
        ...
        ) latest ON true WHERE p.tenant_id=$1 AND p.booking_id=$2 AND p.superseded_at IS NULL ORDER BY p.created_at,p.id`,
          [actor.tenantId, bookingId],
        )
      ).rows;
    });
  ...
  private async validateParty(
    tx: Parameters<Database["transaction"]>[1] extends (tx: infer T) => unknown
      ? T
      : never,
    actor: Actor,
    bookingId: string,
    ...
    const actual = Object.entries(
      roster.passengers.reduce<Record<string, number>>((counts, passenger) => {
        counts[passenger.category] = (counts[passenger.category] ?? 0) + 1;
        return counts;
      }, {}),
    ).sort(([left], [right]) => left.localeCompare(right));
    if (JSON.stringify(expected) !== JSON.stringify(actual))
      throw new ConflictException(
        "Passenger categories and counts must match the reserved party",
      );
  }
    ...
    return this.db.command(
      ...
      async (tx) => {
        ...
        await record(
          ...
          bookingId,
          null,
          { count: result.length, rosterVersion: 1 },
        );
        return result;
      },
    );
  ...
  correct(actor: Actor, bookingId: string, key: string, raw: unknown) {
    ...
    return this.db.command(
      actor,
      `booking.passengers.correct:${bookingId}`,
      key,
      input,
      async (tx) => {
        await this.validateParty(tx, actor, bookingId, input);
        const prior = (
          await tx.query(
            "SELECT id,roster_version,name,category,is_minor,identity_pending FROM booking_passengers WHERE tenant_id=$1 AND booking_id=$2 AND superseded_at IS NULL FOR UPDATE",
            [actor.tenantId, bookingId],
          )
        ).rows;
        if (!prior.length)
          throw new ConflictException(
            "Passenger roster must be recorded before it can be corrected",
          );
        const rosterVersion =
          Math.max(...prior.map((row) => row.roster_version)) + 1;
        const result = input.passengers.map((passenger) => ({
          id: randomUUID(),
          ...passenger,
          rosterVersion,
        ...
        if (!supersededBy)
          throw new ConflictException("Passenger roster cannot be empty");
        for (const passenger of result)
          await tx.query(
            "INSERT INTO booking_passengers(tenant_id,id,booking_id,name,category,is_minor,identity_pending,roster_version) VALUES($1,$2,$3,$4,$5,$6,$7,$8)",
            [
        ...
        for (const passenger of prior)
          await tx.query(
            "UPDATE booking_passengers SET superseded_at=clock_timestamp(),superseded_by=$3 WHERE tenant_id=$1 AND id=$2",
            [actor.tenantId, passenger.id, supersededBy],
          );
        await record(
          tx,
          actor,
          "booking.passengers_corrected",
          bookingId,
          prior,
          result,
          input.reason,
  ...
  checkIn(actor: Actor, passengerId: string, key: string, raw: unknown) {
    ...
    return this.db.command(
      actor,
      `passenger.checkin:${passengerId}`,
      key,
      input,
      async (tx) => {
        const passenger = (
          await tx.query(
            `SELECT p.id,p.booking_id,p.identity_pending,b.departure_id,b.state,(h.quote->>'totalMinor')::bigint AS total_minor,
        ...
        WHERE p.tenant_id=$1 AND p.id=$2 AND p.superseded_at IS NULL FOR UPDATE`,
        ...
        if (isFieldCrewRole(actor.role)) {
          ...
          if (!assignment.rowCount)
            throw new BadRequestException(
              "Crew can check in only passengers on their assigned departure",
            );
        }
        const prior = (
          await tx.query(
        ...
        const result = {
          ...
          state,
          balanceSettled,
          waiverSigned: passenger.waiver_signed,
        };
        await tx.query(
          "INSERT INTO passenger_checkins(tenant_id,id,passenger_id,state,actor_id) VALUES($1,$2,$3,$4,$5)",
          [actor.tenantId, result.id, passengerId, state, actor.actorId],
        );
        await record(
          tx,
          actor,
  ...
  issueToken(actor: Actor, passengerId: string, key: string) {
    return this.db.command(
      actor,
      `passenger.token:${passengerId}`,
      key,
      {},
      async (tx) => {
        const passenger = await tx.query(
          "SELECT id FROM booking_passengers WHERE tenant_id=$1 AND id=$2 AND superseded_at IS NULL",
          [actor.tenantId, passengerId],
        );
        ...
        await tx.query(
          "UPDATE passenger_checkin_tokens SET revoked_at=clock_timestamp() WHERE tenant_id=$1 AND passenger_id=$2 AND revoked_at IS NULL",
          [actor.tenantId, passengerId],
        );
        await tx.query(
          "INSERT INTO passenger_checkin_tokens(tenant_id,id,passenger_id,token_hash,expires_at,created_by) VALUES($1,$2,$3,$4,$5,$6)",
          [
            actor.tenantId,
            result.id,
            passengerId,
        ...
        await record(
          tx,
          actor,
          "passenger.checkin_token_issued",
          result.id,
          null,
          { passengerId, expiresAt: result.expiresAt },
        );
        return result;
      },
    );
  ...
  resolveToken(actor: Actor, raw: unknown) {
    ...
    return this.db.transaction(actor, async (tx) => {
      const row = (
        await tx.query(
          `SELECT p.id,p.name,p.category,p.is_minor,p.booking_id,b.departure_id FROM passenger_checkin_tokens t JOIN booking_passengers p ON p.tenant_id=t.tenant_id AND p.id=t.passenger_id JOIN bookings b ON b.tenant_id=p.tenant_id AND b.id=p.booking_id WHERE t.token_hash=$1 AND t.revoked_at IS NULL AND t.expires_at>clock_timestamp() AND p.tenant_id=$2 AND p.superseded_at IS NULL`,
          [digest(input.token), actor.tenantId],
        )
      ).rows[0];
      if (!row)
        throw new NotFoundException("Check-in token is invalid or expired");
      const assigned = await tx.query(
        "SELECT 1 FROM departure_assignments WHERE tenant_id=$1 AND crew_actor_id=$2 AND departure_id=$3 AND status='active'",
        [actor.tenantId, actor.actorId, row.departure_id],
      );
      if (!assigned.rowCount)
        throw new BadRequestException(
          "Crew can resolve only passengers on an assigned departure",
        );
      return {
        passengerId: row.id,
        name: row.name,
...
export class PassengerController {
  ...
  @Get("bookings/:id/passengers") @Access("bookings.read") list(
    @CurrentActor() actor: Actor,
    @Param("id") value: string,
  ) {
    return this.service.list(actor, parse(id, value));
  }
  @Post("bookings/:id/passengers") @Access("bookings.write") replace(
    @CurrentActor() actor: Actor,
    @Param("id") value: string,
  ...
  @Post("bookings/:id/passengers/corrections")
  ...
  @Post("passengers/:id/checkin") @Access("checkin.write") checkIn(
    @CurrentActor() actor: Actor,
    @Param("id") value: string,
    @Headers("idempotency-key") key: string,
    @Body() body: unknown,
  ...
  @Post("passengers/:id/checkin-token") @Access("bookings.write") issueToken(
    @CurrentActor() actor: Actor,
    @Param("id") value: string,
    @Headers("idempotency-key") key: string,
  ) {
  ...
  @Post("crew/checkin-token/resolve") @Access("crew.trip.read") resolveToken(
    @CurrentActor() actor: Actor,
    @Body() body: unknown,
  ) {
    return this.service.resolveToken(actor, body);
  }
}
