...
async function main() {
  ...
  try {
    ...
    function sqlLiteral(value: unknown, col?: string, table?: string): string {
      ...
      const udt = table && col ? columnTypes.get(`${table}.${col}`) : undefined;
    ...
    const newTenantId = intoTenantId ?? map.get(sourceTenantId.toLowerCase())!;
      ...
      for (const row of staff) {
        ...
        lines.push(
          `INSERT INTO staff_users(${cols.join(",")}) VALUES (${cols
            .map((c) => sqlLiteral((staffRow as Record<string, unknown>)[c], c))
            .join(",")});`,
        );
        lines.push(
