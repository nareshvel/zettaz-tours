...
**Next owner-directed surface:** remaining **Administration** owner visual (General & branding, Taxes, Guest stays, Finance Partners — 19 Sep toolbar/search pass; still needs owner look). **Profile polish follow-up** shipped 17 Sep (owner visual outstanding). **Audit** and **Document library** polish shipped 17 Sep. Insights **Customers** polish shipped 17 Sep. **Operations** closed pending testing. **Subscription messaging polish (row 17) is held.** Crew store: iOS ASC `6813693098` 1.0.1 (2); Play Internal 1.0.1/2. **Do not start Phase 5.** See [agent-current-sprint.md](../HANDOFF/agent-current-sprint.md).
...
| 16 | **Tenant settings — module pass in progress 15 September 2026; Security empty-state 17 Sep; needs owner visual pass** | Printers & documents rebuilt (agent pairing, per-document printer + paper, job lifecycle); Waiver templates moved to Operations with version-publish edit + guarded delete; Booking integrations sub-tabs; Localization 3-column + formatting actually wired; sidebar setup checklist; Payment integrations rename. Non-owners on Security now see an explicit owner-only empty state instead of a blank card. See [agent-current-sprint.md](../HANDOFF/agent-current-sprint.md) and [print-and-settings-evidence.md](../TESTING/print-and-settings-evidence.md). Earlier pass (11 Sep): Aside facts, `?tab=` sync, short mobile nav labels, sticky saves, support ConfirmDialog; shared country list dropdown; locked Track A currency copy; exclusive tax-rate explanation; denser desktop meta / tighter mobile tab spacing |
...
**Next polish implementation:** remaining **Tenant settings** (row 16), starting with Pickup locations list tools. Operations (11a) closed pending testing. Subscription (17) messaging **held**. Finance cashbook **held:** [finance-money-in-out-later.md](finance-money-in-out-later.md). Authority: [agent-current-sprint.md](../HANDOFF/agent-current-sprint.md).
