...
- **Web UI (14 September 2026):** Matches Plan pickups hierarchy — product title + departure badge, readiness metrics, Needs attention (screen), dispatcher note, numbered **pickup sequence cards** for screen and print. Plan pickups link + icon Print/PDF. No separate desktop-only table.
- Plan pickups (Phase 1): trip header + readiness metrics, Needs attention, smart Add / Add all, reorder, per-stop notes, unsaved/print hygiene. GET `/ops/v1/departures/{id}/pickups` returns `departure` and `exceptions`.
...
Mental model: [pickup-disposition-and-plans.md](./pickup-disposition-and-plans.md).
...
- Crew paper format, required guest identifiers, phone exposure, ship/hotel and all-aboard details, and retention policy remain tenant decisions.
...
- UI polish + demo seed: [pickup-location-modal-and-print-evidence.md](../../TESTING/pickup-location-modal-and-print-evidence.md).
