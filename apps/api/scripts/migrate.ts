...
export async function migrate(adminUrl: string, runtimeRole: string) {
  ...
  try {
    ...
    // Include scripts that ran in this transaction before the failure — they rolled back too.
    const firstNew = appliedThisRun[0];
    const firstUncommitted =
      firstNew !== undefined
        ? names.indexOf(firstNew)
        : failed
          ? names.indexOf(failed)
          : previouslyApplied;
