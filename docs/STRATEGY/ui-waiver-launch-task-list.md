...
| 16 | **Tenant settings — agent polish 15–20 Sep 2026; needs owner visual pass** | Printers & documents rebuilt; Waiver templates lifecycle; Booking integrations sub-tabs + search / empty / tenant timestamps; Localization ISO currency roles; Guest stays search+Add row; hold duration in minutes; support-grant expiry in tenant timezone; Payment integrations restack (Zettaz Pay hero + live methods; waiting-on-platform); **Partners moved Platform → Operations**; Security owner-only empty state; Pickup locations search+add. See [tenant-settings-polish-evidence.md](../TESTING/tenant-settings-polish-evidence.md). |
| 17 | **Subscription — plan picker modal 20 Sep 2026; needs owner visual pass** | Plan grid lives in a Subscribe / Change plan modal, not on the page. Trial/cancelled/unpaid/incomplete → Subscribe; active/past_due → Change plan. Messaging 19 Sep; `.billing-cycle` 20 Sep. See [subscription-polish-evidence.md](../TESTING/subscription-polish-evidence.md). |
