...
const paths: Record<string, RegExp> = {
  GET: /^(crew\/v1\/today|staff\/v1\/(workspace\/(session|departures|reservations|members|roles|summary|briefing|notifications|subscription)|customers(?:\/[a-f0-9-]{36})?|departures\/[a-f0-9-]{36}\/availability|bookings\/[a-f0-9-]{36}(\/(changes|passengers|notifications))?)|admin\/v1\/(tenant(?:\/readiness)?|products(?:\/[a-f0-9-]{36})?|availability-rules(?:\/[a-f0-9-]{36})?|invitations|support-access)|integrations\/v1\/(accounts|mappings|inbox|assisted-imports)|finance\/v1\/(finance-overview|vendors(?:\/[a-f0-9-]{36})?|expense-categories(?:\/[a-f0-9-]{36})?|expenses(?:\/[a-f0-9-]{36}(?:\/payments)?)?|partners(?:\/available|\/[a-f0-9-]{36}\/(?:bookings\/(unsettled|unsettled-summary)|claimable-bookings|ledger|settlements(?:\/[a-f0-9-]{36})?))?|finance-aging|partner-aging|partner-claims|partner-statements|partner-finance-summary|bookings\/[a-f0-9-]{36}\/finance-summary)|ops\/v1\/(audit|board|resources|crew|compliance-documents(?:\/[a-f0-9-]{36}\/file)?|document-library\/usage|assignments|print-templates|print-jobs(?:\/[a-f0-9-]{36}\/pdf)?|pickup-locations|waiver-templates|stays\/(options|vessels)|bookings\/[a-f0-9-]{36}\/waivers|passengers\/[a-f0-9-]{36}\/waiver(?:-pdf)?|departures\/[a-f0-9-]{36}\/(manifest|pickups|pickup-list|assignments|rebooking-options)))$/,
  POST: /^(auth\/v1\/(register|change-password|password-recovery\/(request|complete))|admin\/v1\/(tenant\/logo|products(?:\/[a-f0-9-]{36}\/cover)?|schedules|members|staff(?:\/[a-f0-9-]{36}\/grant-access)?|roles|invitations|support-access\/[a-f0-9-]{36}\/(decision|revoke))|integrations\/v1\/(accounts|mappings|assisted-imports|inbox\/[a-f0-9-]{36}\/review)|staff\/v1\/(workspace\/(checkout|billing-portal|switch-plan)|holds|overbook-holds|bookings|bookings\/[a-f0-9-]{36}\/(payments(?:\/[a-f0-9-]{36}\/adjustments)?|confirm|revive-hold|concession|change-quotes|changes|cancel|passengers(?:\/corrections)?|boarding-roster|notifications(?:\/[a-f0-9-]{36}\/retry)?)|passengers\/[a-f0-9-]{36}\/(checkin|checkin-token)|crew\/checkin-token\/resolve)|crew\/v1\/departures\/[a-f0-9-]{36}\/events|finance\/v1\/(vendors|expense-categories|expenses\/batch|expenses(?:\/[a-f0-9-]{36}\/payments(?:\/[a-f0-9-]{36}\/void)?)?|partners(?:\/[a-f0-9-]{36}\/(?:status|bookings|settlements))?|partner-claims(?:\/[a-f0-9-]{36}\/decision)?)|ops\/v1\/(resources|crew|compliance-documents|assignments|print-templates|print-jobs(?:\/[a-f0-9-]{36}\/outcome)?|pickup-locations|waiver-templates|stays\/(vessels|accommodations)|bookings\/[a-f0-9-]{36}\/(waivers|checkin)|passengers\/[a-f0-9-]{36}\/waiver|departures\/[a-f0-9-]{36}\/(pickups|operational-status|start|unstart|itinerary|rebooking-preview|rebook)))$/,
  PATCH:
